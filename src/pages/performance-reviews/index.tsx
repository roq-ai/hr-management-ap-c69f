...
import { getPerformanceReviews, deletePerformanceReviewById } from 'apiSdk/performance-reviews';
import { PerformanceReviewInterface } from 'interfaces/performance-review';

type ColumnType = ColumnDef<PerformanceReviewInterface, unknown>;

interface PerformanceReviewListPageProps {
...
export function PerformanceReviewListPage(props: PerformanceReviewListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getPerformanceReviews({
        ...
        order: params.order,
        searchTermKeys: ['reviewer.contains', 'comments.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PerformanceReviewInterface>>(
    () => `/performance-reviews?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deletePerformanceReviewById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: PerformanceReviewInterface) => {
    if (hasAccess('performance_review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/performance-reviews/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/performance-reviews/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('performance_review', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/performance-reviews/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('performance_review', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="performance_review" />
        </Flex>

        {hasAccess('performance_review', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/performance-reviews/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
