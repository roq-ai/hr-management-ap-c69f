...
import { getBenefitById } from 'apiSdk/benefits';
import { BenefitInterface } from 'interfaces/benefit';

function BenefitViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<BenefitInterface>(
    () => (id ? `/benefits/${id}` : null),
    () =>
      getBenefitById(id, {
        relations: ['employee'],
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('benefit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/benefits/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                ...
                <FormListItem label="Eligibility Criteria" text={data?.eligibility_criteria} />
