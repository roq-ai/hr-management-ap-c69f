...
import { getBenefitById, updateBenefitById } from 'apiSdk/benefits';
...
function BenefitEditPage() {
  ...
  const handleSubmit = async (values: BenefitInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateBenefitById(id, values);
      mutate(updated);
      resetForm();
