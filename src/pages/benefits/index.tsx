...
import { getBenefits, deleteBenefitById } from 'apiSdk/benefits';
import { BenefitInterface } from 'interfaces/benefit';

type ColumnType = ColumnDef<BenefitInterface, unknown>;
...
export function BenefitListPage(props: BenefitListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getBenefits({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'description.contains', 'eligibility_criteria.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BenefitInterface>>(
    () => `/benefits?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: BenefitInterface) => {
    if (hasAccess('benefit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/benefits/view/${row.id}`);
    }
  };
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'eligibility_criteria', header: 'Eligibility Criteria', accessorKey: 'eligibility_criteria' },
    {
      id: 'start_date',
      header: 'Start Date',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/benefits/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('benefit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/benefits/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('benefit', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="benefit" />
        </Flex>

        {hasAccess('benefit', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/benefits/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
