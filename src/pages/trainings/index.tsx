...
type ColumnType = ColumnDef<TrainingInterface, unknown>;
...
export function TrainingListPage(props: TrainingListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getTrainings({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'description.contains', 'location.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TrainingInterface>>(
    () => `/trainings?params=${JSON.stringify(params)}`,
    fetcher,
  );
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/trainings/view/${record.id}`} passHref legacyBehavior>
                <Button
