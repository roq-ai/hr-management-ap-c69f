...
import { getPerformanceReviews } from 'apiSdk/performance-reviews';
import { PerformanceReviewInterface } from 'interfaces/performance-review';
import { getEmployees } from 'apiSdk/employees';
import { EmployeeInterface } from 'interfaces/employee';

import { PerformanceReviewListPage as ShowCaseList } from 'pages/performance-reviews';

function DashboardPage() {
  const { session } = useSession();

  const trainingStat = useSWR<PaginatedInterface<TrainingInterface>>(
    () => `/trainings`,
    () => getTrainings({ limit: 1, offset: 0 }),
  );

  const performanceReviewStat = useSWR<PaginatedInterface<PerformanceReviewInterface>>(
    () => `/performance-reviews`,
    () => getPerformanceReviews({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Trainings',
      result: trainingStat?.data?.totalCount,
      isLoading: trainingStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Performance Reviews',
      result: performanceReviewStat?.data?.totalCount,
      isLoading: performanceReviewStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Employees',
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`performance-reviews`} passHref legacyBehavior>
                  <Button
