...
export function EmployeeListPage(props: EmployeeListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getEmployees({
        relations: ['user', 'benefit.count', 'performance_review.count', 'training.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['job_title.contains', 'department.contains'],
        ...(params.filters || {}),
      }),
  ...
  const columns: ColumnType[] = [
    { id: 'employee_number', header: 'employee_number', accessorKey: 'employee_number' },
    {
      id: 'hire_date',
      header: 'Hire Date',
      accessorKey: 'hire_date',
      cell: ({ row: { original: record } }: any) =>
        record?.hire_date ? format(parseISO(record?.hire_date as unknown as string), 'dd-MM-yyyy') : '',
    },
    { id: 'job_title', header: 'Job Title', accessorKey: 'job_title' },
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('benefit', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'benefit',
          header: 'Benefit',
          accessorKey: 'benefit',
          cell: ({ row: { original: record } }: any) => record?._count?.benefit,
        }
      : null,
    hasAccess('performance_review', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'performance_review',
          header: 'Performance Review',
          accessorKey: 'performance_review',
          cell: ({ row: { original: record } }: any) => record?._count?.performance_review,
        }
      : null,
    hasAccess('training', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          cell: ({ row: { original: record } }: any) => record?._count?.training,
        }
      : null,
