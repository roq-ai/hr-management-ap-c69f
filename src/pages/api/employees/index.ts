...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createEmployee() {
    ...
    const body = { ...req.body };
    if (body?.benefit?.length > 0) {
      const create_benefit = body.benefit;
      body.benefit = {
        create: create_benefit,
      };
    } else {
      delete body.benefit;
    }
    if (body?.performance_review?.length > 0) {
      const create_performance_review = body.performance_review;
      body.performance_review = {
        create: create_performance_review,
      };
    } else {
      delete body.performance_review;
    }
